...
### 왜 Google OAuth로 로그인하는가
이메일 기반 회원가입은 이메일 발송 의존성이 생기고 rate limit, 스팸 처리 등 운영 부담이 따른다. Google OAuth는 인증을 Google에 위임하므로 이메일 SMTP 설정이 불필요하고, 사용자 입장에서도 별도 비밀번호 없이 기존 Google 계정으로 즉시 가입할 수 있다. Supabase에서 Google provider를 활성화하는 것만으로 연동된다.

### 왜 Resend를 Auth 이메일 SMTP로 사용할 예정인가 _(예정)_
Supabase의 기본 내장 이메일 서비스는 **프로젝트 전체 기준 시간당 3개** 발송으로 제한된다. 이메일 기반 부가 기능(구독 취소 링크 등) 발송이 필요한 경우를 대비해 연동 예정이다. Resend 무료 플랜은 100개/일(3,000개/월)을 지원하며, Supabase SMTP 설정에서 Host를 `smtp.resend.com`으로 교체하는 것만으로 연동된다.