  ...
  return (
    <div className="bg-slate-50 text-slate-900 min-h-screen">
      ...
      <nav className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Link
              href="/auth/login"
              className="text-sm font-semibold px-4 py-2 hover:bg-slate-100 rounded-full transition"
            >
              로그인
            </Link>
            <a
      ...
      <main className="pt-24 pb-20 max-w-7xl mx-auto px-6">
          ...
          {ARTICLES.map((article) => (
            <div key={article.title} className="group cursor-pointer">
              <div className="relative overflow-hidden rounded-2xl mb-4 bg-slate-200 aspect-[4/3]">
                ...
                />
                {article.premium && (
                  <div className="absolute top-4 right-4 bg-white/90 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest shadow-sm">
                    Premium
                  </div>
                )}
              </div>
        ...
        {/* CTA Section */}
        <section className="mt-24 bg-indigo-900 rounded-[3rem] p-12 text-center text-white relative overflow-hidden shadow-2xl shadow-indigo-200">
          <div className="relative z-10 max-w-2xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link
                href="/auth/signup"
                className="px-8 py-4 bg-white text-indigo-700 font-bold rounded-2xl hover:bg-indigo-50 transition text-sm"
              >
                무료로 시작하기
              </Link>
              <Link
                href="/auth/login"
                className="px-8 py-4 bg-indigo-700/60 text-white font-semibold rounded-2xl hover:bg-indigo-700 transition text-sm border border-indigo-600"
              >
                로그인
              </Link>
            </div>
            <p className="text-xs text-indigo-300 mt-6">신용카드 없이 무료로 시작하세요.</p>
          </div>
